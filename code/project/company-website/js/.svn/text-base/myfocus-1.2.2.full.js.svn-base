/*
* myFocus JavaScript Library v1.2.2
	...
	Method={
		...
		fixIMG:function(box,maxWidth,maxHeight){
			...
			for(var i=0;i<imgs.length;i++){
				...
				if(IMG[M1]>MAX){//缩小
					imgs[i][M1] = MAX;
					imgs[i][M2] = (IMG[M2] * MAX) / IMG[M1];
				} else {//实际显示
					imgs[i][M1] = IMG[M1];
					imgs[i][M2] = IMG[M2];
				}
				if(maxHeight>imgs[i].height){//垂直居中
