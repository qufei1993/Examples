/*
* myFocus JavaScript Library v1.0.0 Fianl
* JS焦点图库 myFocus-1.0正式版
* 
...
var myFocus={
	...
	animate:function(obj,prop,val,spd,type,fn){
		var opa=prop=='opacity'?true:false,offset=this.style(obj,prop),am=typeof val==='string';
		if(opa&&this.style(obj,'display')=='none'){obj.style.display='block';this.opa(obj,0);}
		var t=0,b=opa?this.opa(obj):parseInt(offset=='auto'?0:offset),c=am?val/1:val-b,d=spd||50,st=type||'easeOut',m=c>0?'ceil':'floor';
	...
	initCSS:function(p){
		...
		if(p.wrap) this.wrap([this.$(p.id)],p.style+'_wrap');
	...
	removeClass:function(o,name){var oClass=o.className;var reg="/\\s*"+name+"\\b/g";o.className=oClass?oClass.replace(eval(reg),''):''},
	alterSRC:function(o,name,del){var img=this.$$('img',o)[0];img.src=del?img.src.replace(eval("/"+name+"\\.(?=[^\\.]+$)/g"),'.'):img.src.replace(/\.(?=[^\.]+$)/g,name+'.')},
	setting:function(par,im){
		par.S=par.style+'-'+par.id;
		if(this[par.style]){for(var p in par) this[par.style][p]=par[p];par=this[par.style];}
		if(im){myFocus.$(par.id).className+=' '+par.style+' '+par.S;myFocus.initCSS(par);myFocus[par.style](par);return;}
		if(window.attachEvent){(function(){try{myFocus.$(par.id).className+=' '+par.style+' '+par.S;myFocus.initCSS(par);}catch(e){setTimeout(arguments.callee,0);}})();window.attachEvent('onload',function(){myFocus[par.style](par)});}
　　		else{document.addEventListener("DOMContentLoaded",function(){myFocus.$(par.id).className+=' '+par.style+' '+par.S;myFocus.initCSS(par);},false);window.addEventListener('load',function(){myFocus[par.style](par)},false);}
	},
...
myFocus.setting.prototype={param:function(style,p){this[style]=p}};